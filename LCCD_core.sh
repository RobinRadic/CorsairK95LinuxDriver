...
####
# if activelayout is not set, set it to Admin
###
if [ ! -f $directory/activelayout ]; then
...
####
# check and load config, print error if does not exist
###
if [ -f $directory/LCCD_conf ]; then
...
##########
##########  f_debug         ## Debug mode helps tracing where crashes occur (if $debug = 1) - must be declared soon
##########
function f_debug {
    if [ "$debug" = "1" ]; then
        echo "########################################"
...
##########
##########  f_is_running      ## Check if a function is allready running (in order to prevent it to run twice at the same time)
##########
function f_is_running {
...
##########
##########  f_confirm         ## Confirm that you want to start a function (rsync, ...) - I linked it with G12
##########
function f_confirm {
...
##########
##########  f_select_latest         ## to be done -- would allow the selection of certain files according to their extensions $1= "avi|mkv|mp4" for example
##########
function f_select_latest {
...
##########
##########  f_libnotify         ## manage user interaction through libnotify if enabled, else echoes its output
##########
function f_libnotify {
...
##########
##########  f_switch         ## Switch between keyboard layouts - use "./LCCD_core.sh switch"
##########
function f_switch {
    fonction=f_switch
    f_debug $fonction
...
##########
##########  f_create         ## Creates a blank keyboard layout - use "././LCCD_core.sh create nameofprofile"
##########
function f_create {
    fonction=f_create
    f_debug $fonction
...
##########
##########  f_cheatsheet        ## Generates a list of the functions available in currently active config, based on _-_
##########
function f_cheatsheet {
...
source $directory/LCCD_Triggers_K95.sh #Loads the right triggers for device
exit